...
fun Application.configureRouting() {
    ...
    routing {
        get("/") {
            call.respondText("API telah berjalan. Dibuat oleh Iel Oceline.")
        }
        ...
        // Route Workouts
        route("/workouts") {
            get { workoutService.getAllWorkouts(call) }
            post { workoutService.createWorkout(call) }
            get("/{id}") { workoutService.getWorkoutById(call) }
            put("/{id}") { workoutService.updateWorkout(call) }
            delete("/{id}") { workoutService.deleteWorkout(call) }
            get("/{id}/image") { workoutService.getWorkoutImage(call) }
        }
